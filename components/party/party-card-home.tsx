...
const PartyCardHome = async ({ party }: PartyCardHomeProps) => {
  ...
  return (
    <Link className="flex flex-col justify-center w-60 h-28 shadow-md shadow-zinc-200 border rounded-xl" href={`/party/${party.slug}`}>
      <HStack className="w-full h-full ">
        <div className="h-full w-4/12">
          <div className=' bg-slate-100 rounded-l-xl flex items-center justify-center  w-full h-full'>
            <Logo size={48} className='text-slate-500' />
          </div>
        </div>
        <VStack className="w-8/12 justify-between p-2">
